                  ...
                  return (
                    <tr key={item._id || index} className="border-b border-border/60 hover:bg-gray-50/50 transition border-opacity-50">
                      ...
                      <td className="py-4 px-5">
                        <span className={`px-3 py-1 rounded text-[11px] font-bold ${item.verificationStatus === 'verified' ? 'bg-[#E1F7E3] text-[#1E7145]' :
          ...
          <div className="flex items-center gap-1.5">
            ...
            {[...Array(totalPages)].map((_, i) => (
              <button
                key={i}
                onClick={() => setPage(i + 1)}
                className={`w-7 h-7 rounded flex items-center justify-center text-xs transition ${page === i + 1
