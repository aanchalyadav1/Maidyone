...
export const Verification = () => {
  ...
  // Single stable fetch function — no duplicate mount calls
  ...
  useEffect(() => {
    const t = setTimeout(() => fetchWorkers(), query ? 300 : 0);
      ...
      <div className="bg-white rounded-[24px] border border-border shadow-sm overflow-hidden">
        <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div className="flex gap-4">
            ...
            <select className="border border-border rounded-lg px-3 py-2 text-[13px] text-gray-600 outline-none shadow-sm bg-white">
                  ...
                  <tr key={w._id} className="border-b border-gray-50 hover:bg-gray-50/50 transition last:border-0">
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        <img
                          src={w.user?.avatar || `https://i.pravatar.cc/150?img=${(idx % 70) + 1}`}
                          className="w-9 h-9 rounded-full border border-gray-200 shadow-sm object-cover"
                          alt="Worker"
                          onError={e => {
                            (e.target as HTMLImageElement).src = `https://i.pravatar.cc/150?img=${(idx % 70) + 1}`;
                          }}
                        />
                        <div>
                    ...
                    <td className="py-4 px-6">
                      <button
                        className="flex items-center gap-2 text-blue-600 font-bold text-[12px] bg-blue-50 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition disabled:opacity-50 disabled:cursor-not-allowed"
                    ...
                    <td className="py-4 px-6">
                      <span className="bg-[#FFF4E5] text-[#ED6C02] text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded flex items-center gap-1.5 w-max">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#ED6C02]" /> Pending
                      </span>
                    </td>
                    <td className="py-4 px-6">
        ...
        <div className="p-4 border-t border-gray-100 flex justify-between items-center text-gray-500 text-[13px]">
          <span>
            Showing <span className="font-bold text-gray-800">{pendingCount}</span> pending request{pendingCount !== 1 ? 's' : ''}
          </span>
          <div className="flex gap-2">
            <button className="border border-border rounded px-3 py-1 hover:bg-gray-50 disabled:opacity-50" disabled>Previous</button>
            <button className="border border-border rounded px-3 py-1 hover:bg-gray-50 disabled:opacity-50" disabled>Next</button>
          </div>
        </div>

        {error && <div className="text-red-600 font-semibold text-center py-2 px-4">{error}</div>}
      </div>
    </div>
  );
};
